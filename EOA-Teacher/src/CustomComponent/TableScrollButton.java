...
public class TableScrollButton extends JLayeredPane {
    ...
    private class ScrollLayerUI extends LayerUI<JScrollPane> {
        ...
        private Shape shape;
        private final Color color = new Color(60, 148, 225);
        private boolean mousePressed;
        private boolean mouseHovered;
        private final Image image = new ImageIcon(getClass().getResource("/UI/Image/MiniUp.png")).getImage();

        @Override
        public void installUI(JComponent c) {
            super.installUI(c);
            if (c instanceof JLayer jLayer) {   jLayer.setLayerEventMask(AWTEvent.MOUSE_EVENT_MASK | AWTEvent.MOUSE_MOTION_EVENT_MASK); }
        }

        @Override
        public void uninstallUI(JComponent c) {
            if (c instanceof JLayer jLayer) {   jLayer.setLayerEventMask(0);    }   
            super.uninstallUI(c);
        }
